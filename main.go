...
func main() {
	...
	go func() {
		...
		isAPMLambdaMode := conf.APMLambdaMode
		logShipLoop(ctx, logServer, telemetryClient, isAPMLambdaMode)
	}()
...
func logShipLoop(ctx context.Context, logServer *logserver.LogServer, telemetryClient *telemetry.Client, isAPMLambdaMode bool) {
	for {
		...
		go telemetryClient.SendFunctionLogs(ctx, invokedFunctionARN, functionLogs, isAPMLambdaMode)
	}
}
